...
1. **[기록 데이터]**: 사진 속 내용에서 식사(식단) 정보나 운동 정보(종목명, 무게 kg, 횟수 reps, 세트 등)를 파악하여 텍스트 데이터 형식으로 정형화해 작성해 주세요. 
   - 예시 (식사): `저녁 식단: 닭가슴살 샐러드, 현미밥`
   - 예시 (운동): `운동: 벤치프레스 60kg 10회 3세트`
   - 식단과 운동 정보가 모두 없거나 분석할 수 없다면 이 영역은 비워두세요.
